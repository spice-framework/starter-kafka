// Command starter-kafka-release builds deterministic signed source releases.
package main
...
import (
	...
	starterrelease "github.com/spice-framework/starter-kafka/internal/release"
)
...
func run(ctx context.Context, arguments []string, stdout, stderr io.Writer) int {
	flags := flag.NewFlagSet("starter-kafka-release", flag.ContinueOnError)
	...
	if flags.NArg() != 0 {
		return writeExit(stderr, 2, "starter-kafka-release: unexpected arguments: %s\n", strings.Join(flags.Args(), " "))
	}
	if version == "" {
		return writeExit(stderr, 2, "starter-kafka-release: -version is required\n")
	}
	if rehearsal && signingKey != "" {
		return writeExit(stderr, 2, "starter-kafka-release: -rehearsal is unsigned and cannot use -signing-key\n")
	}
	resolvedEpoch, err := sourceEpoch(ctx, root, epoch)
	if err != nil {
		return writeExit(stderr, 1, "starter-kafka-release: %v\n", err)
	}
	if !rehearsal {
		...
		if epochErr != nil {
			return writeExit(stderr, 1, "starter-kafka-release: read HEAD source epoch: %v\n", epochErr)
		}
		if !resolvedEpoch.Equal(headEpoch) {
			return writeExit(stderr, 1, "starter-kafka-release: source epoch %d differs from tagged commit epoch %d\n", resolvedEpoch.Unix(), headEpoch.Unix())
		}
	}
	var key []byte
	if signingKey != "" {
		...
		if err != nil {
			return writeExit(stderr, 1, "starter-kafka-release: read signing key: %v\n", err)
		}
	}
	...
	if _, err := fmt.Fprintf(stdout, "starter-kafka release %s created %d artifact(s) in %s.\n", version, len(result.Files), result.OutputDir); err != nil {
		return 1
	}
	return 0
}
