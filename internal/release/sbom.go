...
func buildSBOM(ctx context.Context, root, version string, epoch time.Time) ([]byte, error) {
	...
	document := spdxDocument{
		...
		Name:              "Spice Kafka Starter " + version,
		DocumentNamespace: "https://github.com/spice-framework/starter-kafka/releases/" + version + "/spdx/" + hex.EncodeToString(namespaceHash[:]),
		CreationInfo: spdxCreationInfo{Created: epoch.UTC().Format(time.RFC3339), Creators: []string{
			"Organization: Spice Framework", "Tool: github.com/spice-framework/starter-kafka/cmd/starter-kafka-release",
		}},
