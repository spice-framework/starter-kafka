...
func releaseParity(ctx context.Context, root string) error {
	parent, err := os.MkdirTemp("", "starter-kafka-release-parity-*")
	if err != nil {
...
func validateReleaseParity(
	...
	base := "starter-kafka_" + strings.TrimPrefix(rehearsalVersion, "v")
	...
	if err := validateReleaseArchiveParity(
		...
		"starter-kafka_"+strings.TrimPrefix(rehearsalVersion, "v")+"/",
		filepath.Join(retainedRoot, archiveName),
		"starter-kafka-"+strings.TrimPrefix(rehearsalVersion, "v")+"/",
	); err != nil {
		return err
	}
...
func validateReleaseSBOMParity(centralContent, retainedContent []byte) error {
	...
	baseNamespace := "https://github.com/spice-framework/starter-kafka/releases/" +
	...
	if !found {
		return errors.New("central release SBOM has no exact Kafka root package")
	}
...
func validateCentralSBOMProvenance(document releaseSBOM, baseNamespace string) error {
	if document.Name != "starter-kafka "+rehearsalVersion ||
...
func validateRetainedSBOMProvenance(document releaseSBOM, baseNamespace string) error {
	if document.Name != "Spice Kafka Starter "+rehearsalVersion ||
		...
		return errors.New("retained release SBOM provenance does not match the Kafka builder")
	}
	return nil
}
