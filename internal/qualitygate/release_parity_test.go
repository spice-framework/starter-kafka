...
func TestValidateReleaseArchiveParity(t *testing.T) {
	...
	writeParityTestArchive(t, central, "starter-kafka_0.0.0-rehearsal/", "same", "")
	writeParityTestArchive(t, retained, "starter-kafka-0.0.0-rehearsal/", "same", "")
	if err := validateReleaseArchiveParity(
		central,
		"starter-kafka_0.0.0-rehearsal/",
		retained,
		"starter-kafka-0.0.0-rehearsal/",
	); err != nil {
	...
	t.Run("rejects entry drift", func(t *testing.T) {
		...
		writeParityTestArchive(t, drifted, "starter-kafka-0.0.0-rehearsal/", "different", "")
	...
	t.Run("rejects unexpected retained root", func(t *testing.T) {
		...
		writeParityTestArchive(t, wrong, "starter-kafka_0.0.0-rehearsal/", "same", "")
		err := validateReleaseArchiveParity(
			central,
			"starter-kafka_0.0.0-rehearsal/",
			wrong,
	...
	t.Run("rejects hidden decompressed tail", func(t *testing.T) {
		...
		prefix := "starter-kafka_0.0.0-rehearsal/"
...
func paritySBOMFixtures() (releaseSBOM, releaseSBOM) {
	...
	central := common
	central.Name = "starter-kafka " + rehearsalVersion
	...
	retained := common
	retained.Name = "Spice Kafka Starter " + rehearsalVersion
	retained.DocumentNamespace = parityNamespace("", 'b')
	retained.CreationInfo.Creators = []string{
		"Organization: Spice Framework",
		"Tool: github.com/spice-framework/starter-kafka/cmd/starter-kafka-release",
	}
...
func writeParityTestRelease(
	...
	t.Helper()
	root := t.TempDir()
	prefix := "starter-kafka-0.0.0-rehearsal/"
...
func releaseArchiveName() string {
	return "starter-kafka_" + strings.TrimPrefix(rehearsalVersion, "v") + "_source.tar.gz"
}

func releaseSBOMName() string {
	return "starter-kafka_" + strings.TrimPrefix(rehearsalVersion, "v") + "_sbom.spdx.json"
}

func parityNamespace(extra string, digit rune) string {
	return "https://github.com/spice-framework/starter-kafka/releases/" + rehearsalVersion +
