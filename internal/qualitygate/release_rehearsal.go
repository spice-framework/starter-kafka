...
func releaseRehearsal(ctx context.Context, root string) error {
	parent, err := os.MkdirTemp("", "starter-kafka-release-rehearsal-*")
	if err != nil {
	...
	plan, err := capture(ctx, root, offlineVendor, "go", "tool", developmentTool,
		"library-release", "plan", "--root="+root, "--repo=starter-kafka",
...
func validateReleaseSBOM(content []byte) error {
	...
	prefix := "https://github.com/spice-framework/starter-kafka/releases/" + rehearsalVersion + "/spdx/v1/"
	if sbom.SPDXVersion != "SPDX-2.3" || sbom.DataLicense != "CC0-1.0" || sbom.SPDXID != "SPDXRef-DOCUMENT" ||
		sbom.Name != "starter-kafka "+rehearsalVersion || !validSBOMNamespace(sbom.DocumentNamespace, prefix) ||
